...
class BottleNeck(nn.Module):
    def __init__(self,
        ...
        if bidirectional: 
            self.rnn = nn.LSTM(hidden_dim, hidden_dim//2, batch_first=True, bidirectional=bidirectional)
        else:
...
class DeepVQES(nn.Module):
    ...
    def __init__(self,
        ...
        encoder_mic_channels = [1, 8, 16, 20, 24]
        encoder_ref_channels = [1, 8]
        bottleneck_channels = 24 * 6
        decoder_channels = [24, 20, 16, 8, 1]
        ...
        for idx in range(num_layers):
            if idx < 1:
                self.mic_encoders.append(
                    EncoderBlock(encoder_mic_channels[idx], encoder_mic_channels[idx+1], causal=casual)
                )
                self.ref_encoders.append(
                    EncoderBlock(encoder_ref_channels[idx], encoder_ref_channels[idx+1], causal=casual) 
                )
            elif idx == 1:
                self.mic_encoders.append(
                    EncoderBlock(encoder_mic_channels[idx]+encoder_ref_channels[idx], encoder_mic_channels[idx+1], causal=casual)
                )
            else:
                self.mic_encoders.append(
                    EncoderBlock(encoder_mic_channels[idx], encoder_mic_channels[idx+1], causal=casual)
        ...
        for idx in range(num_layers):
            if idx != num_layers - 1:
                self.decoders.append(
                    DecoderBlock(decoder_channels[idx], decoder_channels[idx+1], upscale_factor=2, is_last=False, causal=casual)
                )
            elif idx == 0:
                self.decoders.append(
...
def test_model():
    ...
    model.eval()
    cpt = torch.load("/home/node25_tmpdata/xcli/percepnet/c_aec/checkpoint/rnnvqe_last.pt.tar", map_location="cpu")
    ...
    sf.write("/home/node25_tmpdata/xcli/percepnet/c_aec/test_wav/out_last_py.wav", outputs, 16000)
